...
static coap_pdu_code_t map_message_1_status_to_response(
    const enum edh_message_1_handler_status status) {
  switch (status) {
  case EDH_MSG1_HDL_OK:
  ...
  case EDH_MSG1_HDL_ERR_EDHOC_MESSAGE_2_COMPOSE_FAILED:
  default:
...
static const char* message_1_status_to_error_string(
  ...
  case EDH_MSG1_HDL_ERR_PAYLOAD_TOO_LARGE:
...
coap_pdu_code_t cp_srv_map_message_1_result_to_coap(
  ...
  if (message_1_result.status != EDH_MSG1_HDL_OK) {
