 ...
 * @since 12/05/2026
 * @brief Definitions for the srv_coap_m1_parse_result_builders module.
...
struct srv_coap_extract_message_1_result srv_coap_parse_message_1_ok(
  ...
  return (struct srv_coap_extract_message_1_result){
      .status = SRV_COAP_EXTRACT_MSG1_OK,
      .buffer = buffer,
  };
}
...
srv_coap_parse_message_1_invalid_error_buffer(void) {
  return (struct srv_coap_extract_message_1_result){
      .status = SRV_COAP_EXTRACT_MSG1_ERR_INVALID_ERROR_BUFFER,
  };
}
