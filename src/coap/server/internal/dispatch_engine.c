 ...
 */
static bool dispatch_deps_are_valid(const struct cp_srv_dispatch_deps* deps) {
...
void cp_srv_dispatch_post_with_dependencies(
  ...
  const struct cp_srv_parse_edhoc_request_result parse_edhoc_result =
  ...
  if (edhoc_ctx == NULL) {
    ...
    const struct edh_srv_message_1_request request_data = {
    ...
    const struct edh_srv_parse_message_3_result parse_message_3_result =
