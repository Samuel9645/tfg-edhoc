...
static struct srv_edhoc_parameters create_valid_params(void) {
  return tst_edhoc_srv_get_default_params();
}
...
void test_setup_context_ok_with_valid_parameters(void) {
  ...
  const struct srv_edhoc_parameters params = create_valid_params();
...
void test_setup_context_fails_on_null_credentials(void) {
  const struct com_writable_buffer error_buffer = create_valid_error_buffer();
  const struct srv_edhoc_parameters valid_params = create_valid_params();
